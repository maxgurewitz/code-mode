...
#[tokio::test]
async fn code_mode_can_call_openai_codex_fixture_server_with_api_token_auth_arg() -> Result<()> {
    let api_token = "token_fixture_cli_arg";
    let api_token_for_backend = api_token.to_owned();
    let listener = TcpListener::bind("127.0.0.1:0").await?;
    let addr = listener.local_addr()?;
    let backend = tokio::spawn(async move {
        let (mut stream, _) = listener.accept().await.expect("accept");
        ...
        assert_eq!(
            request.headers.get("authorization").map(String::as_str),
            Some(format!("Bearer {api_token_for_backend}").as_str())
        );
        ...
        write_response(
            ...
            "200 OK",
            "text/event-stream",
            &sse_body("Hello through code-mode token auth"),
        )
        .await
        .expect("response");
    });
...
[servers.openai-codex]
command = "cargo"
args = ["run", "-q", "--manifest-path", {manifest_path:?}, "--", "--auth-mode", "api_token"]
env = {{ OPENAI_CODEX_MCP_API_TOKEN = {api_token:?}, OPENAI_CODEX_MCP_BASE_URL = {base_url:?}, OPENAI_CODEX_MCP_LOG = "error" }}
"#,
        manifest_path = manifest_path.display().to_string(),
        api_token = api_token,
        base_url = format!("http://{addr}"),
    ...
    assert_eq!(
        result["structuredContent"]["text"],
        json!("Hello through code-mode token auth")
    );
